...
# Load per-cell-line DESeq RDS files and combine.
# Expected files: a549_pb_deseq_tables.rds, k562_pb_deseq_tables.rds, mcf7_pb_deseq_tables.rds
# Each has columns: log2FoldChange, lfcSE, pvalue, gene, pb (drug name), cell_line
...
# as "{source}_{target}" (e.g. "a549_k562").
...
if (sys.nframe() == 0) {
  ...
  eval_dir     <- default_arg(args, "eval-dir", file.path(project_path, "results/eval/sciplex"))
  ...
  run_trade_eval_correlations(
    ...
    de_dir     = default_arg(args, "de-dir", file.path(project_path, "data/sigs/sciplex")),
    output_dir = default_arg(args, "output-dir", file.path(project_path, "results/plots/trade_metric_correlations/sciplex")),
