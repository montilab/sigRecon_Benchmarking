...
trade_one <- function(tbl, n_sample = NULL) {
  ...
  out <- tryCatch(
    TRADEtools::TRADE(
      mode = "univariate", results1 = trade_input,
      log2FoldChange = "log2FoldChange", lfcSE = "lfcSE", pvalue = "pvalue",
...
# Load limma DE tables from CSV files produced by 01_DrugMatrix_sigs.R.
# Files: {drug}_kidney_sigs.csv and {drug}_liver_sigs.csv in de_dir.
# limma topTable columns: X (gene, from rownames), logFC, AveExpr, t, P.Value, adj.P.Val, B
# TRADE requires log2FoldChange, lfcSE (= logFC / t), pvalue.
load_drugmatrix_de <- function(de_dir) {
  tissues <- c("kidney", "liver")
  map_dfr(tissues, function(tissue) {
    csvs <- Sys.glob(file.path(de_dir, paste0("*_", tissue, "_sigs.csv")))
    if (length(csvs) == 0) {
      message("No CSVs found for tissue: ", tissue, " in ", de_dir)
      return(tibble())
    }
    map_dfr(csvs, function(f) {
      drug_name <- str_remove(basename(f), paste0("_", tissue, "_sigs\\.csv$"))
      tbl <- read.csv(f, stringsAsFactors = FALSE) |> as_tibble()
      # After write.csv with row.names=TRUE the gene column is named "X"
      gene_col <- if ("X" %in% names(tbl)) "X" else rownames(tbl)
      tbl |>
        rename(gene = all_of(if ("X" %in% names(tbl)) "X" else character(0))) |>
        mutate(
          gene           = as.character(gene),
          log2FoldChange = logFC,
          lfcSE          = abs(logFC / t),   # SE = logFC / t-statistic
          pvalue         = P.Value,
          cell_line      = tissue,
          drug           = drug_name
        ) |>
        select(gene, log2FoldChange, lfcSE, pvalue, cell_line, drug)
    })
  })
}

compute_trade_metrics <- function(de_dir, n_sample = NULL) {
  if (!requireNamespace("TRADEtools", quietly = TRUE)) {
    stop("TRADEtools is required. Install with remotes::install_github('ajaynadig/TRADEtools').")
  }

  message("Loading DrugMatrix limma DE tables from: ", de_dir)
  combined <- load_drugmatrix_de(de_dir) |>
    mutate(context_key = normalize_context(cell_line))

  if (nrow(combined) == 0) stop("No DE data loaded from: ", de_dir)
...
# Join TRADE metrics to an eval table. The source column encodes both contexts
# as "{source}_{target}" (e.g. "kidney_liver").
add_trade_deltas <- function(eval_tbl, trade_metrics) {
  eval_tbl |>
    mutate(
      delta_NES  = NES_TRUE  - NES_FALSE,
      delta_jacc = jacc_TRUE - jacc_FALSE,
      source_ctx = str_extract(source, "^[^_]+"),
      target_ctx = str_extract(source, "[^_]+$"),
      source_key = normalize_context(source_ctx),
      target_key = normalize_context(target_ctx)
    ) |>
    left_join(
      trade_metrics |> select(
        source_key = context_key, gene = drug,
        source_trade_context = context, source_n_genes_trade = n_genes_trade,
        source_TI = TI, source_pi_DEG = pi_DEG, source_trade_error = trade_error
      ),
      by = c("source_key", "gene")
    ) |>
    left_join(
      trade_metrics |> select(
        target_key = context_key, gene = drug,
        target_trade_context = context, target_n_genes_trade = n_genes_trade,
        target_TI = TI, target_pi_DEG = pi_DEG, target_trade_error = trade_error
      ),
      by = c("target_key", "gene")
    ) |>
...
spearman_summary <- function(df, x_col, y_col, group_col = NULL) {
  ...
  if (is.null(group_col)) {
    cor_group(df) |> mutate(group = "pooled", .before = 1)
  } else {
    df |>
      group_by(.data[[group_col]]) |>
      group_modify(~ cor_group(.x)) |>
      ungroup() |>
...
run_trade_eval_correlations <- function(eval_paths, de_dir, output_dir, n_sample = NULL) {
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

  trade_cache <- file.path(output_dir, "context_trade_metrics.rds")
  if (file.exists(trade_cache)) {
    message("Loading cached TRADE metrics: ", trade_cache)
    trade_metrics <- readRDS(trade_cache)
  } else {
    trade_metrics <- compute_trade_metrics(de_dir, n_sample = n_sample)
    saveRDS(trade_metrics, trade_cache)
  }
  ...
  for (eval_path in eval_paths) {
    ...
    joined <- readRDS(eval_path) |> as_tibble() |> add_trade_deltas(trade_metrics)
    saveRDS(joined, file.path(output_dir, paste0(label, "_trade_joined.rds")))

    for (x_col in c("delta_TI", "source_TI", "target_TI")) {
      for (y_col in c("delta_NES")) {
        pooled <- spearman_summary(joined, x_col, y_col) |>
          mutate(eval_table = label, x_metric = x_col, y_metric = y_col, scope = "pooled", .before = 1)
        by_target <- spearman_summary(joined, x_col, y_col, "target_ctx") |>
        ...
        plot_scatter(joined, x_col, y_col, "target_ctx",
                     paste(label, x_col, "vs", y_col), "Faceted by target tissue",
                     paste0(base, "_by_target.png"), TRUE)
      }
    }
  }
...
if (sys.nframe() == 0) {
  ...
  eval_dir     <- default_arg(args, "eval-dir", file.path(project_path, "results/eval/drugmatrix"))
  ...
  run_trade_eval_correlations(
    ...
    de_dir     = default_arg(args, "de-dir", file.path(project_path, "data/sigs/drugmatrix/tables")),
    output_dir = default_arg(args, "output-dir", file.path(project_path, "results/plots/trade_metric_correlations/drugmatrix")),
