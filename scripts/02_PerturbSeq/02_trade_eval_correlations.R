...
# Load per-cell-line DESeq RDS files and combine into a single table.
# Expected files: k562_pb_deseq_tables.rds, rpe1_pb_deseq_tables.rds
# Each has columns: log2FoldChange, lfcSE, pvalue, gene, pb (perturbation), cell_line
compute_trade_metrics <- function(de_dir, n_sample = NULL) {
  ...
  rds_files <- list.files(de_dir, pattern = "_pb_deseq_tables\\.rds$", full.names = TRUE)
  if (length(rds_files) == 0) stop("No *_pb_deseq_tables.rds files found in: ", de_dir)

  message("Loading DESeq tables from: ", de_dir)
  ...
  combined |>
    ...
    mutate(
      ...
      context_key = normalize_context(cell_line)
    ) |>
    group_split(context_key, drug) |>
    map_dfr(trade_one, n_sample = n_sample)
}

# Join TRADE metrics to an eval table. For bidirectional datasets the source
# column encodes both contexts as "{source}_{target}" (e.g. "k562_rpe1").
    ...
    for (x_col in c("delta_TI", "source_TI", "target_TI")) {
      for (y_col in c("delta_NES")) {
        ...
        plot_scatter(joined, x_col, y_col, "target_ctx",
                     paste(label, x_col, "vs", y_col), "Faceted by target cell line",
...
if (sys.nframe() == 0) {
  ...
  eval_dir     <- default_arg(args, "eval-dir", file.path(project_path, "results/eval/perturb-seq"))
  ...
  run_trade_eval_correlations(
    ...
    de_dir     = default_arg(args, "de-dir", file.path(project_path, "data/sigs/perturb-seq")),
    output_dir = default_arg(args, "output-dir", file.path(project_path, "results/plots/trade_metric_correlations/perturb_seq")),
