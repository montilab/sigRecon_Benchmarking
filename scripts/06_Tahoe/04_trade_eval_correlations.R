...
# pi_DEG = 3 * n_genes / kappa; pi_DEG > n_genes only when kappa < 3 (platykurtic),
# which real transcriptome-wide effect distributions essentially never are (TRADE
# paper: leptokurtic, median pi_DEG ~45 genome-wide, ~500 for essential genes).
# Verified directly against raw SciPlex pseudobulk DE tables: ashr's default optimizer
# (mixsqp) sometimes converges to a degenerate single-component mixture (kappa lands
# exactly at 1.8, the kurtosis of a continuous uniform) at essentially the same
# log-likelihood as a proper multi-component fit -- a numerical-optimization artifact,
# not signal. n_active <= 2 catches the same failure mode even when it doesn't push
# pi_DEG above n_genes.
...
trade_one <- function(tbl, n_sample = NULL) {
  ...
  failed <- function(msg) tibble(context = context, context_key = context_key, drug = drug, n_genes_trade = nrow(trade_input), TI = NA_real_, pi_DEG = NA_real_, null_weight = NA_real_, kappa = NA_real_, trade_flag = NA_character_, trade_error = msg)
  ...
  tibble(context = context, context_key = context_key, drug = drug, n_genes_trade = n_genes, TI = TI, pi_DEG = pi_DEG, null_weight = null_weight, kappa = kappa, trade_flag = trade_flag, trade_error = NA_character_)
}
